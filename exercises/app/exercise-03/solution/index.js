...
// RATKAISU 1/4: Memoisoi getTemperature-funktio. Käytä cache-avaimena esimerkiksi
// funktion parametreista muodostettua merkkijonoa, kuten tässä.
const getTemperatureMemoized = _.memoize(getTemperature, (...args) => _.values(args).join("_"))

const getTemperatures = async (coordinates) => {
  const temperatures = await Promise.all(coordinates.map(async ({lat, long}) => {
    // RATKAISU 2/4: Käytä memoisoitua getTemperatureMemoized-funktiota
    const temperature = await getTemperatureMemoized(lat, long)
    return {temperature}
  }))
  return temperatures;
}
...
app.get('/temperature/country/:countryCode', async (req, res) => {
  ...
  if (row) {
    ...
    // RATKAISU 3/4: Käytä memoisoitua getTemperatureMemoized-funktiota
    const temperature = await getTemperatureMemoized(lat, long)
...
app.get('/temperature/country/:countryCode/capital', async (req, res) => {
  ...
  if (row) {
    ...
    // RATKAISU 4/4: Käytä memoisoitua getTemperatureMemoized-funktiota
    const temperature = await getTemperatureMemoized(row.lat, row.long)
